...
namespace Bit
{
											...
											const Version & p_Version )
	{
		...
		if( m_Open == true )
		{
			BitLog::NewEngine(Log::Error) << "The graphic device is already loaded." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "The render output is not loaded." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Binding opengl extensions failed." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Failed to load default shaders." << Log::End;
		}
	...
	void OpenGLGraphicDeviceWin32::Close( )
	{
		...
		if( m_Context )
		{
			...
			{
				BitLog::NewEngine(Log::Error) << "Can not release the context." << Log::End;
			}

			// Delete the context
			if( !wglDeleteContext( m_Context ))
			{
				BitLog::NewEngine(Log::Error) << "Can not delete the context." << Log::End;
			}
	...
	*/
												...
												const Version & p_Version )
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Can not choose pixel format." << Log::End;
			return false;
		}
		if( ( SetPixelFormat( p_RenderOutput.GetDeviceContextHandle( ), PixelFormat, &PFD ) ) == false )
		{
			BitLog::NewEngine(Log::Error) << "Can not set pixel format." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Can not create a regular OpenGL context." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Can not get the function for creating the context." << Log::End;
			return false;
		}
	...
	Bool OpenGLGraphicDeviceWin32::LoadDefaultShaders( )
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Failed to compile InitialPoseShader vertex shader." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Failed to compile model fragment shader." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Failed to compile VertexAnimationShader vertex shader." << Log::End;
			return false;
		}
