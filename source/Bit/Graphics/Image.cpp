...
namespace Bit
{
	...
	Bool Image::LoadFromFile( const std::string & p_Filename )
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Not supporting JPG images yet." << Log::End;
			return false;
		}
	...
	Bool Image::LoadFromTgaFile( const std::string & p_Filename )
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Not an uncompressed true color image." << Log::End;
		...
		{
			BitLog::NewEngine(Log::Error) << "Wrong pixel depth format: " << (Int32)m_PixelDepth << " bytes." << Log::End;
			delete tga.GetData( ); // Delete the data by hand
			return false;
		}
	...
	Bool Image::LoadFromPngFile( const std::string & p_Filename )
	{
		...
		if( m_PixelDepth != 3 && m_PixelDepth != 4 )
		{
			BitLog::NewEngine(Log::Error) << "Wrong pixel depth format: " << m_PixelDepth << " bytes." << Log::End;
	...
	void Image::BgrToRgb( )
	{
		...
		if( m_pData == NULL )
		{
			BitLog::NewEngine(Log::Error) << "Image not containing any data." << Log::End;
			return;
		}
