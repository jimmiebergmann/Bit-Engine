			...
			const std::string & p_Identifier)
		{
			...
			while (timeout.AsMicroseconds() > 0)
			{
				...
				if (buffer[0] == PacketType::Accept)
				{
					...
					BitLog::NewEngine(Log::Error) << timerVector[recvConnectSequence].GetTime().AsMicroseconds() << "   " << roundTimeHalf << Log::End;
			...
			m_Thread.Execute([this]()
			{
				...
				while (IsConnected())
				{
					...
					case PacketType::EntityDestroyed:
					{
						...
						if (pEntity == NULL)
						{
							BitLog::NewEngine(Log::Error) << "Trying to destroy NULL entity." << Log::End;
							continue;
						}
