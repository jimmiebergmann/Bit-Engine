...
namespace Bit
{
	...
	Bool TcpListenerWin32::Listen( TcpSocket & p_TcpSocket )
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Can not listen for clients. Error: " << static_cast<Int32>(GetLastError()) << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Can not accept the client. Error: " << static_cast<Int32>(GetLastError()) << Log::End;
			return false;
		}
