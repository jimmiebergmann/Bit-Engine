...
namespace Bit
{
	...
	Bool Torrent::Tracker::SendRequest( Response & p_Response, const Torrent & p_Torrent, const Time & p_Timeout )
	{
		...
		{
			bitLogNetErr(  "Failed to send request." );
			return false;
		}
		...
		{
			bitLogNetErr(  "Failed to parse the response." );
			return false;
		}
...
/*		* std::string m_FailureReason;	///< Reason why the request failed.
	...
	Bool Torrent::ReadTorrentFile( const std::string & p_Filename )
	{
		...
		{
			bitLogNetErr(  "Could not parse the torrent file." );
			return false;
		}
		...
		{
			bitLogNetErr(  "Could not get the info field" );
			return false;
		}
		...
		if( pieceSize <= 0 )
		{
			bitLogNetErr(  "Could get the piece size." );
			return false;
		}
		...
		if( pieceData.size( ) == 0  )
		{
			bitLogNetErr(  "Could get the piece data." );
			return false;
		}
		...
		{
			bitLogNetErr(  "Error in piece data." );
			return false;
		}
		...
		if( beFiles.GetType( ) == Bencode::Value::List )
		{
			...
			for( SizeType i = 0; i < beFiles.GetSize( ); i++ )
			{
				...
				if( path.size( ) == 0 )
				{
					bitLogNetErr(  "Error in multifile path." );
					return false;
				}

				if( length <= 0 )
				{
					bitLogNetErr(  "Error in multifile length." );
					return false;
				}
		...
		else
		{
			...
			if( filename.size( ) == 0 )
			{
				bitLogNetErr(  "Error in single file name." );
				return false;
			}

			if( length <= 0 )
			{
				bitLogNetErr(  "Error in single file length." );
				return false;
			}
		...
		{
			bitLogNetErr(  "Could not get the info dictionary string." );
			return false;
		}
