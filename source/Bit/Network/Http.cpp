...
namespace Bit
{
	...
	Bool Http::SendRequest(const Request & p_Request, Response & p_Response, const Address & p_Address)
	{
		...
		{
			BitLog::NewEngine(Log::Error) << "Could not connect to the server." << Log::End;
			return false;
		}
		...
		{
			BitLog::NewEngine(Log::Error) << "Could not send the request." << Log::End;
			return false;
		}
		...
		while (headerComplete == false) // Keep on handling the header until we're done/fail.
		{
			...
			if (receiveSize <= 0)
			{
				BitLog::NewEngine(Log::Error) << "Could not receive the header data." << Log::End;
				return false;
			}
			...
			while (1)
			{
				...
				if (firstHeaderLine)
				{
					...
					{
						BitLog::NewEngine(Log::Error) << "Could not parse the protocol." << Log::End;
						return false;
					}
				...
				{
					BitLog::NewEngine(Log::Error) << "Could not parse the body data." << Log::End;
					return false;
				}
			}
		...
		if (headerComplete == false)
		{
			...
			BitLog::NewEngine(Log::Error) << "Could not receive the entire header." << Log::End;
			tcp.Disconnect();
			return false;
